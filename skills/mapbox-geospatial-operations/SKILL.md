...
- As the crow drives - matrix_tool (5x5 matrix, one API call, returns actual route distances)
...
| Within X minutes drive       | isochrone_tool then point_in_polygon_tool             | Need routing for travel-time zone, then geometric containment |
...
- Just need to know its northeast - bearing_tool
...
**Small operations (less than 100 calculations):**
...
**Large operations (more than 1,000 calculations):**

- Geometric tools: May need optimization but still fast
- Routing APIs: Definitely use batch tools (matrix_tool handles up to 25x25)
...
**Why wrong:** As the crow flies is not equal to as the crow drives
...
1. **Does distance mean As the crow flies â‰  as the crow drives?**
